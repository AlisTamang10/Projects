...
if (isset($_POST['submit'])) {
	...
	$status = 1;
	if (
		!empty($fullname) and !empty($mobile) and !empty($email)
		and !empty($age) and !empty($gender) and !empty($blodgroup)
		and !empty($address) and !empty($message) and !empty($temp)
	) {
		if (strlen($mobile) >= 10) {
			if (strlen($temp) > 6) {
				$password = md5($temp);
			...
			<div class="login px-4 mx-auto mw-100">
				...
				<form action="#" method="post" name="signup" onsubmit="return checkpass();">
					...
					<div class="form-group">
						...
						<input type="email" name="emailid" class="form-control" required="true" placeholder="Email Id">
					</div>
					<div class="form-group">
						<label class="mb-2">Age</label>
						<input type="text" class="form-control" name="age" id="age" required="true" placeholder="Age" required="">
					</div>
					<div class="form-group">
